...
export const useActionableErrorMap = (
  ...
  const gotToSupport = () => setShowSupport(true)
  const gotToCredentials = () => dispatch({ type: ActionTypes.ACTIONABLE_ERROR_LOG_IN_AGAIN })
  ...
  const messagingMap: Record<string, ActionableErrorMapEntry> = useMemo(
    () => ({
      [ACTIONABLE_ERROR_CODES.NO_ELIGIBLE_ACCOUNTS]: {
        ...
        primaryAction: { label: __('Log in again'), action: gotToCredentials },
      ...
      [ACTIONABLE_ERROR_CODES.NO_ACCOUNTS]: {
        ...
        secondaryActions: { label: __('Get help'), action: gotToSupport },
      },
      [ACTIONABLE_ERROR_CODES.ACCESS_DENIED]: {
        title: __('Additional permissions needed'),
        primaryAction: { label: __('Review instructions'), action: gotToCredentials },
        secondaryActions: { label: __('Get help'), action: gotToSupport },
      },
