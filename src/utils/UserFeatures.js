...
export const chooseExperimentFeatureRandomly = (features) => {
  ...
  const validFeatures = features.filter(
    (feature) => feature.traffic_percentage
  );
  /* 
  ...
  */
  const maxPercent = validFeatures.reduce(
    (acc, feature) => acc + feature.traffic_percentage,
    0
  );
  const percentBuckets = validFeatures.map(
    (feature) => feature.traffic_percentage / maxPercent
  );
  ...
  */
  for (
    let featureIndex = 0;
    featureIndex < percentBuckets.length;
    featureIndex++
  ) {
