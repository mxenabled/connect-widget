...
describe('Support component tests', () => {
  ...
  it.skipIf(!FeatureToggles.ENABLE_REQUEST_INSTITUTION)(
    'renders the RequestInstitution',
    async () => {
...
it.skipIf(!FeatureToggles.ENABLE_REQUEST_INSTITUTION)(
  'renders the success page after submitting a request via the menu',
  async () => {
    const menuProps = {
      loadToView: VIEWS.MENU,
      onClose,
    ...
    await userEvent.type(
      screen.getByLabelText(/Institution website/i),
      'http://institution.name.com',
    )
    await userEvent.click(continueButton)
