...
const RenderConnectStep = (props) => {
  ...
  const step = useSelector(
    (state) =>
      state.connect.location[state.connect.location.length - 1]?.step ??
      STEPS.SEARCH
  );
  const connectedMembers = useSelector(getMembers);
  const currentMember = useSelector(getCurrentMember);
  const selectedInstitution = useSelector(
    (state) => state.connect.selectedInstitution
  );
  const updateCredentials = useSelector(
    (state) => state.connect.updateCredentials
  );
  const verifyMemberError = useSelector((state) => state.connect.error);
  const showConnectGlobalNavigationHeader = useSelector(
  ...
  const showSupport =
    widgetProfile.enable_support_requests && mode === AGG_MODE;
  ...
  const isDeleteInstitutionOptionEnabled =
    widgetProfile?.display_delete_option_in_connect ?? true;
