...
export const TimeOutDialog = (props) => {
  ...
  // Redux State
  const session_timeout = useSelector(
    (state) => state.profiles.widgetProfile.session_timeout || 900
  ); // Default to 15 minutes
  const widget_type = useSelector((state) => state.profiles.widgetProfile.type);
  const keepalive_url = useSelector(
    (state) => state.profiles.widgetProfile.keepalive_url || null
  );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("message", handleInboundPostMessage);
  });
    ...
    const keepAliveInterval = setInterval(() => {
      if (!idleTimer.isIdle()) {
        const pingUrl = keepalive_url
          ? `${keepalive_url}?_=${Date.now()}&ping_id=${
              window.app.options || ""
            }`
          : null;
